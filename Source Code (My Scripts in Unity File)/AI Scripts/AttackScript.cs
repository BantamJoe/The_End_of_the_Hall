...
public class AttackScript : MonoBehaviour {
	...
	void FixedUpdate (){
		...
		if (distance < enemyLookDistance && (sightAngle < 60 || playerSeen) ) {
			...
			if (distance > enemyPursueDistance) {
				setMovingAnimation (false, true, false);
				move ();
			} else {
				setMovingAnimation (false, false, true);
			}
		} else {
	...
	void walk(){
		setMovingAnimation (true, false, false);
	...
	void setMovingAnimation(bool walking, bool running, bool attacking){
